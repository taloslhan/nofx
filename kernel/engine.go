...
func GetFullDecisionWithStrategy(ctx *Context, mcpClient mcp.AIClient, engine *StrategyEngine, variant string) (*FullDecision, error) {
	...
	// 5. Build market prices map for validation
...
func parseFullDecisionResponse(aiResponse string, accountEquity float64, btcEthLeverage, altcoinLeverage int, btcEthPosRatio, altcoinPosRatio float64, marketPrices map[string]float64, minRiskRewardRatio float64, minSLDistancePct float64) (*FullDecision, error) {
	...
	if err := validateDecisions(decisions, accountEquity, btcEthLeverage, altcoinLeverage, btcEthPosRatio, altcoinPosRatio, marketPrices, minRiskRewardRatio, minSLDistancePct); err != nil {
		return &FullDecision{
			CoTTrace:  cotTrace,
			Decisions: decisions,
...
func validateDecisions(decisions []Decision, accountEquity float64, btcEthLeverage, altcoinLeverage int, btcEthPosRatio, altcoinPosRatio float64, marketPrices map[string]float64, minRiskRewardRatio float64, minSLDistancePct float64) error {
	for i := range decisions {
		mp := 0.0
		if marketPrices != nil {
			mp = marketPrices[decisions[i].Symbol]
		}
		if err := validateDecision(&decisions[i], accountEquity, btcEthLeverage, altcoinLeverage, btcEthPosRatio, altcoinPosRatio, mp, minRiskRewardRatio, minSLDistancePct); err != nil {
...
func validateDecision(d *Decision, accountEquity float64, btcEthLeverage, altcoinLeverage int, btcEthPosRatio, altcoinPosRatio float64, marketPrice float64, minRiskRewardRatio float64, minSLDistancePct float64) error {
	...
	if d.Action == "open_long" || d.Action == "open_short" {
		...
		// Use actual market price as entry price for R:R calculation
		// Fallback to estimated price only if market price unavailable
		var entryPrice float64
		if marketPrice > 0 {
			entryPrice = marketPrice
		} else {
			// Fallback: estimate entry price (legacy behavior)
			if d.Action == "open_long" {
		...
		// Validate SL/TP minimum distance from entry price
		if minSLDistancePct > 0 && marketPrice > 0 {
			slDistance := math.Abs(marketPrice-d.StopLoss) / marketPrice * 100
			if slDistance < minSLDistancePct {
				return fmt.Errorf("stop loss too close to current price (%.4f%%, must be >=%.2f%%) [price: %.2f, stop_loss: %.2f]",
					slDistance, minSLDistancePct, marketPrice, d.StopLoss)
			}
			tpDistance := math.Abs(d.TakeProfit-marketPrice) / marketPrice * 100
			if tpDistance < minSLDistancePct {
				return fmt.Errorf("take profit too close to current price (%.4f%%, must be >=%.2f%%) [price: %.2f, take_profit: %.2f]",
					tpDistance, minSLDistancePct, marketPrice, d.TakeProfit)
			}
		}
