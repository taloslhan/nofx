...
export const riskControl = {
  ...
  emergencyCloseLossPct: {
    zh: '紧急平仓亏损阈值（代码强制）',
    en: 'Emergency Close Loss Threshold (CODE ENFORCED)',
    es: 'Umbral de Cierre de Emergencia (CÓDIGO)',
  },
  emergencyCloseLossPctDesc: {
    zh: '基于保证金盈亏百分比，达到该亏损可绕过最小持仓时间',
    en: 'Margin-based PnL threshold that can bypass min hold time',
    es: 'Umbral de PnL sobre margen para saltar la retención mínima',
  },
  minSlDistancePct: {
    zh: '最小止损距离（代码强制）',
    en: 'Min Stop-Loss Distance (CODE ENFORCED)',
    es: 'Distancia Mínima de Stop Loss (CÓDIGO)',
  },
  minSlDistancePctDesc: {
    zh: '止损/止盈与现价的最小百分比距离',
    en: 'Minimum percentage distance from current price to SL/TP',
    es: 'Distancia mínima porcentual del precio actual a SL/TP',
  },
  timeConstraints: {
    zh: '时间约束',
    en: 'Time Constraints',
    es: 'Restricciones de Tiempo',
  },
  timeConstraintsDesc: {
    zh: '用于限制过早平仓、刚平即重开和过早启动回撤监控',
    en: 'Controls early exits, immediate re-entries, and drawdown monitoring start time',
    es: 'Controla cierres tempranos, reaperturas y monitoreo de drawdown',
  },
  minHoldMinutes: {
    zh: '最小持仓时间（代码强制）',
    en: 'Min Hold Time (CODE ENFORCED)',
    es: 'Tiempo Mínimo de Retención (CÓDIGO)',
  },
  minHoldMinutesDesc: {
    zh: 'AI 主动平仓前必须至少持有的分钟数',
    en: 'Minimum minutes a position must be held before AI can actively close it',
    es: 'Minutos mínimos antes de que la IA pueda cerrar activamente la posición',
  },
  cooldownMinutes: {
    zh: '同币种冷却期（代码强制）',
    en: 'Same-Symbol Cooldown (CODE ENFORCED)',
    es: 'Enfriamiento por Símbolo (CÓDIGO)',
  },
  cooldownMinutesDesc: {
    zh: '同一币种平仓后，禁止再次开仓的冷却时间',
    en: 'Cooldown window after closing before the same symbol can be reopened',
    es: 'Ventana de espera tras cerrar antes de reabrir el mismo símbolo',
  },
  minDrawdownCheckMinutes: {
    zh: '最小回撤检查延迟（代码强制）',
    en: 'Min Drawdown Check Delay (CODE ENFORCED)',
    es: 'Retraso Mínimo de Revisión de Drawdown (CÓDIGO)',
  },
  minDrawdownCheckMinutesDesc: {
    zh: '开仓后经过多少分钟才开始检查持仓回撤',
    en: 'How many minutes to wait after entry before drawdown monitoring starts',
    es: 'Cuántos minutos esperar tras la entrada antes de iniciar el monitoreo de drawdown',
  },
  entryRequirements: {
    zh: '开仓要求',
...
export function ts(
  entry: { zh: string; en: string; [k: string]: string } | undefined,
  lang: string
): string {
  if (!entry) return ''
