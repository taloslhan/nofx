...
} from '../components/trader/ConnectionTestFeedback'
...
export function SettingsPage() {
  ...
  useEffect(() => {
    setModelTestStates((prev) =>
      Object.fromEntries(
        configuredModels
          .map((model) => [model.id, prev[model.id]])
                ...
                <div className="space-y-2">
                  {configuredModels.map((model) => {
                    ...
                    return (
                      <div
                        key={model.id}
                        className="rounded-xl bg-zinc-800/50 hover:bg-zinc-800 border border-zinc-700/50 transition-colors group p-4 space-y-3"
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="text-left min-w-0">
                              ...
                              <p className="text-xs text-zinc-500 truncate">
                                {model.customModelName || model.provider}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                handleTestSavedModelConnection(model.id)
                              }
                              disabled={testState.status === 'testing'}
                              aria-label={t('testSavedConnection', language)}
                              title={t('testSavedConnectionTooltip', language)}
                              className="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-cyan-500/20 bg-cyan-500/10 text-cyan-300 hover:bg-cyan-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            >
                              {testIcon}
                            </button>
                            <button
                              ...
                              className="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-zinc-700/70 bg-zinc-900/60 text-zinc-400 hover:text-zinc-200 hover:border-zinc-600 transition-colors"
                            >
                              <Pencil size={14} />
                            </button>
                          </div>
