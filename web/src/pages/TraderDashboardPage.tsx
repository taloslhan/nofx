...
export function TraderDashboardPage({
  ...
  const grossRealizedPnL = account?.gross_realized_pnl ?? 0
  const totalFee = account?.total_fee ?? 0
  const totalPnLBreakdown = t('traderDashboard.totalPnLBreakdown', language, {
    gross: formatSignedAmount(grossRealizedPnL),
    fee: formatFeeAmount(totalFee),
    unrealized: formatSignedAmount(account?.unrealized_profit ?? 0),
  })
