...
export interface RiskControlConfig {
  ...
  min_sl_distance_pct?: number // Min SL/TP distance from current price (CODE ENFORCED)
  min_hold_minutes?: number // Min holding time before AI can actively close (CODE ENFORCED)
  emergency_close_loss_pct?: number // Margin-based loss threshold to bypass min hold (CODE ENFORCED)
  cooldown_minutes?: number // Cooldown after closing the same symbol (CODE ENFORCED)
  min_drawdown_check_minutes?: number // Delay before drawdown monitoring starts (CODE ENFORCED)
}
